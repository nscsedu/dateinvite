...
import com.braincraft.social.BuildConfig;
import com.braincraft.social.R;
import com.braincraft.social.activity.manager.TapToFocusManager;
import com.braincraft.social.activity.params.ActivityThemeParams;
import com.braincraft.social.activity.params.FFmpegPreviewActivityParams;
import com.braincraft.social.activity.params.FFmpegRecorderActivityParams;
import com.braincraft.social.activity.params.InteractionParamsI;
import com.braincraft.social.activity.params.RecorderActivityThemeParamsI;
import com.braincraft.social.camera.CameraController;
import com.braincraft.social.camera.CameraControllerI;
import com.braincraft.social.recorder.FFmpegFrameRecorder;
import com.braincraft.social.recorder.MediaClipsRecorder;
import com.braincraft.social.recorder.VideoTransformerTask;
import com.braincraft.social.recorder.params.CameraParams;
import com.braincraft.social.recorder.params.RecorderParamsI;
import com.braincraft.social.ui.CameraPreviewView;
import com.braincraft.social.ui.CameraTapAreaView;
import com.braincraft.social.ui.ProgressSectionsView;
import com.braincraft.social.ui.ViewUtil;
import com.braincraft.social.util.Util;
...
public class FFmpegRecorderActivity extends AbstractDynamicStyledActivity implements
    ...
    public static int  flagsegment =0;

    public static int  checkpress =0;

    public static int  checkcolor =0;


    public static int  record_limit_flag =0;
    ...
    protected Button mNextButton,mDeleteButton;
    protected TextView message,next,next1,recordvideotext;
    protected RelativeLayout rt,rt1,relative_message,record_limit;
    public static int flag=0,maxlimit=0;
    ...
    public void onCreate(Bundle savedInstanceState) {
       ...
       message = (TextView) findViewById(R.id.message);
        recordvideotext =(TextView) findViewById(R.id.recordvideotext);
       ...
       /* mRecordButton.setOnClickListener(new OnClickListener() {
            ...
            public void onClick(View v) {
                if(record_limit_flag==1)
                {

                    record_limit.setVisibility(View.VISIBLE);
                    new Handler().postDelayed(new Runnable() {
                        @Override
                        public void run() {
                            record_limit.setVisibility(View.GONE);
                        }
                    }, 5000);
                }
            }
       ...
       // mNextButton.setColorFilter(themeParams.getToolbarWidgetColor());
        ...
        rt1 = (RelativeLayout) findViewById(R.id.rt1);
        rt1.setOnClickListener(this);

       relative_message = (RelativeLayout) findViewById(R.id.relative_message);
        relative_message.setOnClickListener(this);

        relative_message.setVisibility(View.GONE);


        record_limit = (RelativeLayout) findViewById(R.id.record_limit);
        record_limit.setOnClickListener(this);
      ...
      /*  final Dialog dialog = new Dialog(context);
       ...
       /* AlertDialog.Builder popupBuilder = new AlertDialog.Builder(this);
        ...
        // set title
        alertDialogBuilder.setTitle("Your can say this...");

        // set dialog message
        alertDialogBuilder
                .setMessage("Hi i'm from..(say your city)..." +"\n"+
                ...
                .setPositiveButton("OK",new DialogInterface.OnClickListener() {
                    public void onClick(DialogInterface dialog,int id) {
                        ...
                        recordvideotext.setVisibility(View.GONE);
                        relative_message.setVisibility(View.VISIBLE);
                        new Handler().postDelayed(new Runnable() {
                            @Override
                            public void run() {
                                relative_message.setVisibility(View.GONE);
        ...
        CameraControllerI.Facing cameraFacing =CameraControllerI.Facing.FRONT;
    ...
    public List<Integer> getProgressSections() {
        List<MediaClipsRecorder.Clip> clips = mMediaClipsRecorder.getClips();
         progressList = Lists.newArrayListWithCapacity(
        ...
        for (MediaClipsRecorder.Clip clip : clips) {
           ...
           // Log.i(TAG, "clip.getDurationMillis()1:" + clip.getDurationMillis());
        ...
        if (mMediaClipsRecorder.isRecording()) {
           // Log.i(TAG, "mMediaClipsRecorder:" + mMediaClipsRecorder.getCurrentRecordedTimeMillis());
    ...
    public void onMediaRecorderMaxDurationReached() {
       ...
       /* stopRecording();
        if (mSaveVideoTask == null) {
            Log.i(LOG_TAG, "Saving recording111");
        ...
        if (mSaveVideoTask == null && mCameraController.isCameraOpen()) {
            switch (event.getAction()) {
                case MotionEvent.ACTION_DOWN:
                    //checkpress=1;
                    Log.i(TAG, "checkpress:" + checkpress);
                    ...
                    if(pressFlag==false){
                        ...
                        // applay your action
                        if( button_click==1) {
                          //  mProgressView.setProgressColor(getResources().getColor(R.color.white));
                            checkcolor=1;
                            button_click=0;
    ...
    public void onClick(View v) {
        if (v.getId() == R.id.flash_button) {
            ...
            if(button_click==2)
            {
                ...
                Toast.makeText(getApplicationContext(), " last  video clip deleted...", Toast.LENGTH_LONG).show();
                button_click=0;
            }

          //  mProgressView.setProgressColor(getResources().getColor(R.color.red));

          /*  Log.i(TAG, "clip1:" + progressList.size());
            progressList.remove(progressList.size() - 1);
            Log.i(TAG, "clip2:" + progressList.size());
          ...
          /*  DialogInterface.OnClickListener dialogClickListener = new DialogInterface.OnClickListener() {
            ...
            ab.setMessage("Want to delete the last  video clip?").setPositiveButton("Yes", dialogClickListener)
                    .setNegativeButton("No", dialogClickListener).show();*/

           // flag_delete=0;

           //write code mMediaClipsRecorder.removeLastClip();

           // Toast.makeText(getApplicationContext(), "last item deleted", Toast.LENGTH_LONG).show();
